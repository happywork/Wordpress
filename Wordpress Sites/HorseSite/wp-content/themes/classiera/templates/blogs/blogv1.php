...
<section class="blog-post-section section-pad">
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-9 center-block">
                    <h3 class="text-capitalize"><?php echo $classieraBlogSecTitle; ?><i class="fa fa-leanpub"></i></h3>
                    <p><?php echo $classieraBlogSecDesc; ?></p>
                </div>
            </div>
        </div>
	...
	<?php 
	$blogTemplate = $wpdb->get_results("SELECT `post_id` FROM $wpdb->postmeta WHERE `meta_key` ='_wp_page_template' AND `meta_value` = 'template-blog.php' ", ARRAY_A);
	$blogPermalink ="";
	if(!empty($blogTemplate)){
		$blogPermalink = get_permalink($blogTemplate[0]['post_id']);
	}
	?>
    <div class="view-all text-center">
        <a href="<?php echo $blogPermalink; ?>" class="btn btn-primary btn-style-four"><?php esc_html_e( 'View All', 'classiera' ); ?></a>
    </div>
</section><!-- /.blog post -->
<script>
